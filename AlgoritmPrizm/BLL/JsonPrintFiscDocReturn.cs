...
{
    public class JsonPrintFiscDocReturn
    {
        private static JsonSerializerSettings _settings;

        /// <summary>
        /// Имя поля в которое сохранять
        /// </summary>
        [JsonConverter(typeof(JsonSerializerSettingsFieldNameConverter))]
        public FieldDocNumEn fieldName;

        /// <summary>
        /// Номер фискального документа
        /// </summary>
        public int fiscDocNum;
    ...
    ""fiscDocNum"":365
            ...
            if (_settings == null)
            {
                ...
                _settings.Error = (sender, args) =>
                {
                    if (object.Equals(args.ErrorContext.Member, "note1") &&
                      args.ErrorContext.OriginalObject.GetType() == typeof(JsonPrintFiscDocReturn))
                    {
        ...
        /// <returns></returns>
        public static JsonPrintFiscDocReturn DeserializeJson(string json)
        {
            try
            {
                return JsonConvert.DeserializeObject<BLL.JsonPrintFiscDocReturn>(json, BLL.JsonPrintFiscDocReturn.GetSettings());
            }
            catch (Exception ex)
            {
                ApplicationException ae = new ApplicationException(string.Format("Упали при десериализации объекта JsonSms с ошибкой: {)}", ex.Message));
        ...
        public static string SerializeObject(JsonPrintFiscDocReturn json)
        {
            try
            {
            ...
            {
                ApplicationException ae = new ApplicationException(string.Format("Упали при сериализации объекта JsonSms с ошибкой: {)}", ex.Message));
