...
namespace AlgoritmPrizm.BLL
{
    ...
    ""Message"":""""
            ...
            if (_settings == null)
            {
                ...
                _settings.Error = (sender, args) =>
                {
                    if (object.Equals(args.ErrorContext.Member, "note1") &&
                      args.ErrorContext.OriginalObject.GetType() == typeof(JsonCndResponce))
                    {
        ...
        /// <returns></returns>
        public static JsonCndResponce DeserializeJson(string json)
        {
            try
            {
                return JsonConvert.DeserializeObject<JsonCndResponce>(json, JsonCndResponce.GetSettings());
            }
            catch (Exception ex)
            {
        ...
        public static string SerializeObject(JsonCndResponce json)
        {
            try
            {
