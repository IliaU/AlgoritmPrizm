...
namespace AlgoritmPrizm.Com
{
    public class Web
    {
        ...
        private delegate bool ColbackDocument(JsonGetDocumentsItem Document);
        ...
        /// <returns></returns>
        private static bool FilterReportDocument(JsonGetDocumentsItem Document)
        {
            try
            {
                return true;
        ...
        private static List<JsonGetDocumentsItem> GetDocumentsRestSharp(ColbackDocument ColbackDocumentFilter)
        {
            List<JsonGetDocumentsItem> Rez = new List<JsonGetDocumentsItem>();
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    string url = string .Format("http://172.16.1.102/api/backoffice/document?cols=*&page_no={0}&page_size=10",Page);
                    ...
                    if (doc.data.Count > 0)
                    {
                        ...
                        foreach (JsonGetDocumentsItem item in doc.data)
                        {
